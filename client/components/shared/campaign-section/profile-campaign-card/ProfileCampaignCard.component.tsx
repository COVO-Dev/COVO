    ...
    >
      <CardHeader>
        <CardTitle className="flex text-lg font-semibold justify-between items-center">
          ...
          <span
            className={`text-base ${
              campaignData.status.toLowerCase() === "active"
                ? "text-green-500"
                : campaignData.status.toLowerCase() === "pending"
                ? "text-yellow-500"
                : campaignData.status.toLowerCase() === "completed"
                ? "text-blue-500"
                : "text-gray-500"
            }  `}
          >
        ...
        <div className="col-span-1">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm truncate">
              Target Audience: {campaignData.targetAudience.ageGroups}
            </span>
          </div>
          <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm">
              Focus: {campaignData.geographicFocus.country}
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <Users className="h-4 w-4 min-w-[1em] text-muted-foreground" />
            <span className="text-sm">
              Influencer Type: {campaignData.influencerType}
            </span>
          </div>
        </div>

        <div>
          {campaignData.primaryGoals && (
            <div>
              <div className="flex items-center space-x-2">
                <Flag className="h-4 w-4 min-w-[1em] text-muted-foreground" />
                <span className="text-sm">Goal:</span>
              </div>
              <p className="pl-6 text-sm truncate">
                {campaignData.primaryGoals}
              </p>
            </div>
          )}
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm">
              Start Date:{" "}
              {format(new Date(campaignData.startDate), "PPP", {
                locale: enUS,
              })}
            </span>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm">
              End Date:{" "}
              {format(new Date(campaignData.endDate), "PPP", { locale: enUS })}
            </span>
          </div>
          {/* <div>
